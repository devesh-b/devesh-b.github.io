...
const py_build_plugin = () => {
  ...
  return {
    ...
    configureServer(server) {
      const regenerateGeneratedCss = () => { runGenerateStyles(); };
      ...
      server.watcher.on('all', async (event, filePath) => {
        ...
        if (filePath.includes('/assets/images/')) {
          if (event === 'add' || event === 'change' || event === 'unlink') {
            console.log(`[watcher] Image change: ${event} ${filePath}`);
            try {
              const siteConfig = loadSiteConfig();
        ...
        if (event === 'unlink' && !filePath.includes('/assets/images/')) {
          build();
        }
      });
    },
...
export default defineConfig(async ({ command }) => {
  ...
  // Exclude al-folio legacy dirs; include only generated HTML at root/subdirs
