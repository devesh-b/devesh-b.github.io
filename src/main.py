...
def clean_output(directory):
    ...
    preserved_roots = {
        ".git", ".github", ".cache", ".venv", "assets", "content",
        "dist", "node_modules", "src", "templates", "scripts",
        "config.yaml", "package.json", "package-lock.json",
        "requirements.txt", "vite.config.mjs", "README.md",
        "LICENSE.md", "CONTRIBUTING.md", ".env", ".env.example",
...
def normalize_theme_config(config):
    ...
    default_theme = (
        base.get("default") or base.get("preset") or base.get("name") or "dracula"
    )
...
def write_font_file(config, output_path=GENERATED_FONTS_PATH):
    ...
    default_mono = 'ui-monospace, SFMono-Regular, Menlo, Monaco, Consolas, "Liberation Mono", "Courier New", monospace'
    ...
    lines.append(f"  font-family: var(--font-heading, var(--font-body, {default_body}));")
    lines.append("}")
    lines.append("")

    lines.append("code, pre, kbd, samp {")
    lines.append(f"  font-family: var(--font-mono, {default_mono});")
    lines.append("}")
    lines.append("")
...
def generate_syntax_css(theme, output_path=GENERATED_SYNTAX_PATH):
    try:
        ...
        print(f"Warning: Pygments theme '{theme}' not found. Falling back to '{fallback}'.")
...
def render_page(page_config, html_data, site_config, templates, image_manifest=None, **context_data):
    layout = page_config.get("layout") or "post"
    if layout not in templates:
        available = ", ".join(sorted(templates.keys())) or "none"
        print(f"Error: Template '{layout}' not found. Available: {available}. Skipping.")
        return
...
def _build_picture_element(attrs, manifest_entry):
    ...
    for fmt in format_priority:
        ...
        sources.append(f'<source type="{mime}" srcset="{srcset}" sizes="{sizes_value}">')
    ...
    fallback_variants = sorted(
        (v for v in manifest_entry[fallback_format] if v.get("path") and v.get("width")),
    ...
    fallback_src = (
        fallback_variants[-1]['path']
        if fallback_variants[-1]['path'].startswith("http")
        else f"/{fallback_variants[-1]['path']}"
    )

    fallback_srcset = ", ".join(
        f"{v['path'] if v['path'].startswith('http') else '/' + v['path']} {v['width']}w"
...
def main():
    ...
    if args.generate_styles:
        ...
        print(f"Generated theme, font, and syntax CSS (Pygments theme: {syntax_theme}).")
        return
    ...
    if args.file:
        print(f"Change detected in {args.file}, rebuilding...")
        ...
        if not has_file_changed(args.file):
            print(f"No changes detected in {args.file}; rebuilding anyway.")
        ...
        if page_data is None or html_content is None:
            return
        render_page(page_data, html_content, site_config, templates, image_manifest=image_manifest)
    else:
        ...
        for page in pages:
            render_page(
                page["data"], page["content"], site_config, templates,
                image_manifest=image_manifest, **context_data
            )

        tag_template = templates.get("tags") or templates.get("tags.html")
        if tag_template:
            tag_pages(tag_template, site_config, tags, image_manifest=image_manifest)
        else:
            print("Warning: tags template not found; skipping tag pages.")
